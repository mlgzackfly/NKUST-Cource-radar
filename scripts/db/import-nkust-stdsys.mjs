...
function makeSourceKey({ year, term, campusId, programId, unitId, selectCode, courseCode, courseName }) {
  const keyPart = selectCode || courseCode || courseName;
  return `${year}#${term}:${campusId || ""}:${programId || ""}:${unitId || ""}:${keyPart || ""}`.trim();
}
...
async function upsertCourseFromRow({ meta, row }) {
  const sourceKey = makeSourceKey({
    ...
    programId: row.programId || meta.programId,
    unitId: row.unitId,
  ...
  const course = await prisma.course.upsert({
    ...
    create: {
      ...
      degreeId: row.programId || meta.programId || null,
...
async function main() {
  ...
  const dataRoot = path.resolve(process.cwd(), "data/nkust/stdsys");
  const rootExists = await fs
    .stat(dataRoot)
    .then(() => true)
    .catch(() => false);
  if (!rootExists) {
    throw new Error(`Data folder not found: ${dataRoot}. Run the scraper first (npm run scrape:nkust-stdsys).`);
  }
  ...
  for (const filePath of filtered) {
    ...
    const meta = {
      year: String(req.year || "unknown"),
      term: String(req.term || "unknown"),
      programId: String(req.programId || ""),
    };
