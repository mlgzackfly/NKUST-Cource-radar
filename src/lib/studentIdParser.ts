...
// 系所代碼對應的 className 搜尋關鍵字（用於資料庫搜尋）
// 因為資料庫 department 欄位存的是學院名稱，className 才包含系所名
const DEPARTMENT_CLASS_KEYWORDS: Record<string, string> = {
  "01": "創意設計",
  "03": "先進製造",
  "04": "機電系自動",
  "05": "機電系精機",
...
export function getDepartmentClassKeyword(deptCode: string): string | null {
  return DEPARTMENT_CLASS_KEYWORDS[deptCode] || null;
}

export function getDepartmentCollege(deptCode: string): string | null {
  return DEPARTMENT_TO_COLLEGE[deptCode] || null;
}
